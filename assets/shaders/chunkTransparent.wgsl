...
fn vs_main(in: VertexInput) -> VertexOutput {
    ...
    let texture = textures[in.texture.y];
    let texCoords = vec2<f32>(f32((in.texture.x >> 8u) & 255u) / 255.0 * texture.size, f32(in.texture.x & 255u) / 255.0 * texture.size);
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    return mix(color1, color2, in.blend) * in.color;
}