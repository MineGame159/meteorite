...
static class ImGuiImplCacti {
	class Data {
		...
		public GpuImage fontImage ~ ReleaseAndNullify!(_);
	...
	public static CommandBuffer Render(GpuImage target, ImGui.DrawData* drawData) {
		...
		if (drawData.CmdListsCount > 0) {
			// Begin render pass
			RenderPass pass = Gfx.RenderPasses.New(cmds, "ImGui")
				.Color(target)
				.Begin();

			pass.SetViewport(target.Size, false, true);
			...
			void SetupRenderState() {
				pass.Bind(data.pipeline);
				pass.Bind(vertexBuffer);
				pass.Bind(indexBuffer, sizeof(ImGui.DrawIdx) == 2 ? .Uint16 : .Uint32);
				...
				pass.SetPushConstants(scaleTranslate);
			}
...
#unwarn
						...
						pass.SetScissor(.((.) clipMin.x, (.) clipMin.y), .((.) (clipMax.x - clipMin.x), (.) (clipMax.y - clipMin.y)));

						// Bind DescriptorSet with font or user texture
						if (sizeof(ImGui.TextureID) < sizeof(ImGui.U64)) {
							Internal.FatalError("Problem");
						}

						pass.Bind(0, .SampledImage((.) Internal.UnsafeCastToObject(cmd.TextureId), Gfx.Samplers.Get(.Linear, .Linear)));

						// Draw
						pass.DrawIndexed(cmd.ElemCount, (.) (cmd.IdxOffset + globalIdxOffset), (.) (cmd.VtxOffset + globalVtxOffset));
					}
				}
	...
	public static void CreateFontsTexture() {
		...
		data.fontImage = Gfx.Images.Create("ImGui - Font", .RGBA, .Normal, .(width, height));
		Gfx.Uploads.UploadImage(data.fontImage, pixels);

		io.Fonts.SetTexID(Internal.UnsafeCastToPtr(data.fontImage));
	}
	...
	private static void CreateObjects() {
		...
		// Pipeline
		data.pipeline = Gfx.Pipelines.Create(scope PipelineInfo("ImGui")
