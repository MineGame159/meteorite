	...
	class Image {
		...
		public void Write(StringView path, bool isBgra = false) {
			...
			// Image Header
			{
				...
				WriteBigEndian(cs, (.) width); // Width
				WriteBigEndian(cs, (.) height); // Height
			...
			// Image Data
			{
				...
				int dataLength = width * height * components + height;
				uint8* data = new .[dataLength]*;
				for (int y < height) {
					uint8* row = &data[y * (width * components + 1)];

					row[0] = 0;

					if (isBgra) {
						for (int x < width) {
							uint8* pixel = &this.data[y * (width * components) + x * components];
